...
static void imrsim_rmw_process(struct bio *bio)
{
    __u64       lba;
    void        *page_addr;
    struct page *page;  // RMW过程中，对块的额外开销，用page来备份数据

    if(bio){
        __u8 i;
        for(i=0;i<imrsim_rmw_task.lba_num;i++)
        {
            // 重新派发bio (R and W option to the top track)
            ...
            printk(KERN_INFO "imrsim: write amplification progress: 2.\n");
            imrsim_read_page(bio->bi_bdev, lba, PAGE_SIZE, page);
            // imrsim_write_page(bio->bi_bdev, lba, PAGE_SIZE, page);
            __free_pages(page, 0);
            printk(KERN_INFO "imrsim: write amplification progress: 3.\n");
        }
